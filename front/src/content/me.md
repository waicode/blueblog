...
そんな気持ちを抱えていた中で、きっかけと良いご縁もあって、2011年に株式会社ネットプロテクションズへ入社しました。なぜNet Protectionsに来たのかは、以下の記事が詳しいです。
...
description: 今回の社員は、アプリ開発、インフラ（クラウド）、セキュリティや社内IT、その全てを手がけるITのスペシャリスト、相澤雄大です。
---
::
...
description: SIerにてキャリアをスタートした4名の登壇者たちが、事業会社への転身後に感じた魅力や苦労点を語った。
---
::
...
過去の通例に基づきストレートに準拠しようとすると、構造上または運用上かなり厳しくなるケースもありました。そのため、スコープを意図的に限定するための刷新を行い、その上でセキュリティレベルを上げるための代替手段を模索しました。

アプリ構成・システムアーキテクチャ・セキュリティ基準に加え、社内施設・制度等も見直して、**運用設計を含め約1年半かけて準拠を達成**しました。
